...
{
    public class EvsImageJpegCodec : ContentCodec<Image>
    {
        ...
        public EvsImageJpegCodec(IContentCodec<int> intCodec, IContentCodec<Memory<byte>> bytesCodec, IContentCodec<bool> boolCodec, IContentCodec<NetworkDisplay> ndCodec)
        {
            _intCodec = intCodec;
        ...
        public override int Encode(IBufferWriter<byte> writer, Image value)
        {
            ...
            byte[] bytes = UtilityConvertationImages.ConvertToJpeg(value);
            writtenSize += _bytesCodec.Encode(writer, bytes);

            //EvsManager.Instance.evsSize[value.Target.DisplayId] = bytes.Length;
            return writtenSize;
        }
        ...
        public override int GetSize(Image value)
        {
            ...
            size += _intCodec.GetSize(-1); //width
            size += _intCodec.GetSize(-1); //height
